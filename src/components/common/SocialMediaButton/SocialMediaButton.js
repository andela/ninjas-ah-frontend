...
const SocialMediaButton = ({
  buttonId, name, buttonClass, children, href, size
}) => {
  ...
  const button = {
    twitter: {
      ...
      color: '#50ABF1',
      href: `${reactUrl || defaultUrl}/api/v1/auth/twitter`
    },
    facebook: {
      icon: faFacebookSquare,
      color: '#3A559F',
      href: `${reactUrl || defaultUrl}/api/v1/auth/facebook`
    },
    google: {
      icon: faGoogle,
      color: 'red',
      href: `${reactUrl || defaultUrl}/api/v1/auth/google`,
      image: <img src={`${googleSVG}`} style={{ width: `${size}px`, marginBottom: '-3px' }} />
    }
  };
